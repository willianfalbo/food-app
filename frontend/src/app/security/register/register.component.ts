...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    this.registerForm = new FormGroup(
      ...
      {
        validators: [this.equalPasswords], updateOn: 'change',
      });
    this.navigateTo = this.activatedRoute.snapshot.params['to'] || '/';
  }

  equalPasswords(group: AbstractControl): { [key: string]: boolean } {
  ...
  register() {
    this.userService
      ...
      .subscribe(user =>
        // when success
        this.notificationService.notify(`Registration succeeded!`),
        // when error
  ...
  get emailErrorMessage() {
    if (this.email.errors) {
      if (this.email.errors.emailTaken) {
        return 'Email already exists! Pick another one';
      } else {
        return 'Mandatory field or invalid email';
      }
    }
  }
